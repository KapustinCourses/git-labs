#!/usr/bin/env bash
set -euo pipefail
python3 - << 'PYEOF'
import yaml, sys

try:
    with open(".github/workflows/release.yml") as f:
        w = yaml.safe_load(f)
except Exception as e:
    print(f"yaml_error: {e}")
    sys.exit(0)

jobs = w.get("jobs", {})
docker_job = jobs.get("docker", {})

if not docker_job:
    print("docker_job_missing")
    sys.exit(0)

# check needs: [test]
needs = docker_job.get("needs", [])
if isinstance(needs, str):
    needs = [needs]
if "test" in needs:
    print("needs_test_ok")
else:
    print(f"needs_test_missing: {needs}")

# check permissions.packages: write
perms = docker_job.get("permissions", {})
if isinstance(perms, dict) and perms.get("packages") == "write":
    print("packages_write_ok")
else:
    print(f"packages_write_missing: {perms}")

# check actions
found_buildx = False
found_login = False
found_login_ghcr = False
found_build_push = False

for step in docker_job.get("steps", []):
    uses = step.get("uses", "")
    if "setup-buildx-action@v3" in uses:
        found_buildx = True
    if "login-action@v3" in uses:
        found_login = True
        with_data = step.get("with", {})
        if with_data.get("registry") == "ghcr.io":
            found_login_ghcr = True
    if "build-push-action@v6" in uses:
        found_build_push = True

print("buildx_ok" if found_buildx else "buildx_missing")
print("login_ghcr_ok" if (found_login and found_login_ghcr) else f"login_ghcr_missing: login={found_login}")
print("build_push_ok" if found_build_push else "build_push_missing")
PYEOF

